// show-blocks.mjs — run with: node show-blocks.mjs
...
async function main() {
  ...
  if (anchors.length === 0) {
    console.log("No blocks found yet. Upload some evidence first!");
    return;
  }
  ...
  for (const a of anchors) {
    ...
    if (a.explorerUrl) {
      console.log(`     Explorer   : ${a.explorerUrl}`);
    }
    console.log(`     ${"─".repeat(90)}`);
  }

  console.log(`\n  ✅ Total: ${anchors.length} block(s) across all chains.\n`);
}
